...
# Lista completa de 2048 palabras BIP-39
bip39_words = [
...
chrome_driver_path = "C:\\Users\\Georgiana\\Desktop\\Python\\chromedriver-win64\\chromedriver.exe" # Cambia esto a la ruta de tu ChromeDriver
service = Service(chrome_driver_path)
options = webdriver.ChromeOptions()
options.add_argument("--disable-extensions")  # Desactiva otras extensiones
options.add_argument("--disable-popup-blocking")  # Desactiva bloqueo de popups
...
def abrir_wallet():
    """
    Abre la página de la wallet.
    """
    driver.get("chrome-extension://nkbihfbeogaeaoehlefnkodbefgpgknn/home.html#restore-vault")
    time.sleep(5)  # Esperar a que la página cargue

def probar_combinacion(combinacion_str):
    """
    Prueba una combinación en la wallet.
    """
def ingresar_contrasena():
    """
    Ingresa la contraseña nueva y la confirma.
    """
    try:
         # Localizar el campo de contraseña nueva
        input_contrasena = driver.find_element(By.XPATH, '//*[@id="app-content"]/div/div[2]/div/div/div/form/div[2]/div')
        input_contrasena.clear()
        input_contrasena.send_keys("Olivera505%")
           # Hacer clic en el botón "Restaurar"
        boton_restaurar = driver.find_element(By.XPATH, '//*[@id="app-content"]/div/div[2]/div/div/div[2]/form/button')
        boton_restaurar.click()
              # Esperar a que la página procese la contraseña
        time.sleep(5)
...
def probar_combinacion(combinacion_str):
    ...
    """
    try:
        # Localizar el campo de entrada de la frase semilla usando el XPath proporcionado
        input_field = driver.find_element(By.XPATH, '//*[@id="app-content"]/div/div[2]/div/div/div/form/div[1]/div[3]')
        input_field.clear()
        input_field.send_keys(combinacion_str)
        
        # Esperar a que la wallet procese la combinación
        time.sleep(5)  # Aumentamos el tiempo de espera
        
        # Verificar si la combinación fue aceptada
        # (Aquí debes ajustar la lógica según cómo la wallet indique que la combinación es válida)
        if "Wallet unlocked" in driver.page_source:  # Ejemplo de verificación
            return True
        else:
            return False
    except Exception as e:
        print(f"Error al probar la combinación: {e}")
        return False

def generar_combinaciones_con_apple(palabras, palabra_fija="apple", longitud=12, limite=1000):
    """
    Genera combinaciones de 12 palabras que incluyen la palabra "apple".
    """
    count = 0
    # Fijar la palabra "apple" en cada posición posible
    for posicion in range(longitud):
        # Crear una lista temporal sin "apple"
        palabras_temp = [palabra for palabra in palabras if palabra != palabra_fija]
        # Generar combinaciones para las otras 11 palabras
        for combinacion_resto in itertools.permutations(palabras_temp, longitud - 1):
            # Insertar "apple" en la posición actual
            combinacion = list(combinacion_resto)
            combinacion.insert(posicion, palabra_fija)
            combinacion_str = ' '.join(combinacion)
            
            # Probar la combinación en la wallet
            if probar_combinacion(combinacion_str):
                print("¡Combinación válida encontrada!", combinacion_str)
                return combinacion_str  # Detener el proceso si se encuentra una combinación válida
            
            # Si la combinación no funciona, se descarta
            del combinacion, combinacion_str  # Eliminar de la memoria
            count += 1
            if count >= limite:
                print(f"Se alcanzó el límite de {limite} combinaciones.")
                return
...
if __name__ == "__main__":
    try:
        ...
        generador = generar_combinaciones_con_apple(bip39_words, limite=1000)
        for combinacion in generador:
            print("Combinación válida encontrada:", combinacion)
